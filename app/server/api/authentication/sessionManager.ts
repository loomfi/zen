...
export default eventHandler(async event => {
    try {
        ...
        const ip = rbody['ip']
        if (verifier.payload.aud == process.env.jwt_audience && verifier.payload.iss == process.env.jwt_issuer && verifier.payload.exp >= Date.now()/1000 && verifier.payload.id[1] == ip) {
            return {
